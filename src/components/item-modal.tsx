          ...
          >
            ...
            <div className="shrink-0 flex items-center gap-3 border-t border-black/5 bg-white px-5 py-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
              {store.isOpen ? (
                ...
                /* Callejón sin salida resuelto: la tienda cerrada se ve, y el
                   botón da una salida clara (volver a la tienda), en vez de un
                   texto muerto que no hace nada. */
                <motion.button
                  whileTap={{ scale: 0.97 }}
                  onClick={onClose}
                  className="flex flex-1 flex-col items-center justify-center gap-0.5 rounded-full bg-mist px-5 py-2.5 cursor-pointer"
                >
                  <span className="text-[13.5px] font-black text-ink">Tienda cerrada temporalmente</span>
                  <span className="text-[11px] font-bold text-ink-soft">Toca para volver a la tienda</span>
                </motion.button>
              )}
            </div>
          </motion.div>
